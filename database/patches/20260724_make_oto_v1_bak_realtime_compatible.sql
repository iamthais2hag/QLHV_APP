/*
    Lossless V2 -> V1 schema compatibility for the fixed CSDL_OTO_V1_BAK target.
...
IF DB_NAME() <> N'CSDL_OTO_V1_BAK'
    THROW 527540, 'Wrong database. Expected CSDL_OTO_V1_BAK.', 1;
IF NOT EXISTS
(
    SELECT 1
    FROM sys.databases
    ...
    THROW 527541, 'CSDL_OTO_V1_BAK must be ONLINE and read-write.', 1;
IF OBJECT_ID(N'dbo.DM_DonViGTVT', N'U') IS NULL
   OR OBJECT_ID(N'dbo.NguoiLX_HoSo', N'U') IS NULL
    THROW 527542, 'A required CSDL_OTO_V1_BAK compatibility table is missing.', 1;
GO
BEGIN TRY
    BEGIN TRANSACTION;
    IF NOT EXISTS
    (
        ...
        THROW 527543, 'dbo.DM_DonViGTVT.TenDV has an incompatible definition.', 1;
    IF EXISTS
    (
        SELECT 1
        FROM sys.columns
    ...
    (
        ...
        THROW 527544, 'dbo.DM_DonViGTVT.CoQuanQL has an incompatible definition.', 1;
    IF EXISTS
    (
        SELECT 1
        FROM sys.columns
    ...
    (
        ...
        THROW 527550, 'dbo.NguoiLX_HoSo.QDThucHanhHinh has an incompatible definition.', 1;
    IF NOT EXISTS
    (
        SELECT 1
        FROM sys.columns columnMetadata
        ...
        THROW 527551, 'dbo.NguoiLX_HoSo.SoGiayCNTN must remain nvarchar(30) NULL.', 1;
    COMMIT TRANSACTION;
END TRY
BEGIN CATCH
    IF @@TRANCOUNT > 0
...
BEGIN TRY
    ...
    BEGIN
        ...
        (
            ...
            THROW 527552, 'A V2-only dbo.GiaoVien compatibility column is incompatible.', 1;
        IF EXISTS
        (
            SELECT 1
            FROM sys.columns
