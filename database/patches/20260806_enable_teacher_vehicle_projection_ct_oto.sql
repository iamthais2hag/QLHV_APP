...
IF NOT EXISTS(SELECT 1 FROM sys.database_recovery_status WHERE database_id=DB_ID() AND database_guid=CONVERT(uniqueidentifier,N'9A8B9BC1-18F3-4823-8123-3DC197A9D540'))
    THROW 532710,'TVP_OTO_DATABASE_GUID_REJECTED',1;
IF NOT EXISTS(SELECT 1 FROM sys.change_tracking_databases WHERE database_id=DB_ID())
    THROW 532711,'TVP_OTO_DATABASE_CT_DISABLED',1;
IF EXISTS
(
    ...
    THROW 532712,'TVP_OTO_SOURCE_IDENTITY_REJECTED',1;
...
IF (SELECT COUNT(*) FROM sys.change_tracking_tables WHERE object_id IN(OBJECT_ID(N'dbo.GiaoVien'),OBJECT_ID(N'dbo.XeTap'),OBJECT_ID(N'dbo.KhoaHoc_GiaoVien'),OBJECT_ID(N'dbo.KhoaHoc_XeTap')) AND is_track_columns_updated_on=1)<>4
    THROW 532713,'TVP_OTO_CT_VERIFY_FAILED',1;
SELECT N'TEACHER_VEHICLE_PROJECTION_OTO_CT_PASS' Marker,CHANGE_TRACKING_CURRENT_VERSION() AnchorVersion;
