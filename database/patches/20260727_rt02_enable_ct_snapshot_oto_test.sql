...
IF @ExpectedDatabase = N'EXACT_TEST_DB'
   ...
   OR @ExpectedDatabaseGuid IS NULL
    THROW 527270, 'RT02 template placeholders must be replaced from an approved isolated environment.', 1;

IF DB_NAME() <> @ExpectedDatabase
    THROW 527271, 'RT02 exact isolated OTO database guard failed.', 1;
...
(
    ...
    THROW 527272, 'ISOLATED_DATABASE_IDENTITY_REJECTED: production database name.', 1;

IF CAST(SERVERPROPERTY(N'ServerName') AS nvarchar(128)) <> @ExpectedServer
    THROW 527273, 'ISOLATED_DATABASE_IDENTITY_REJECTED: server identity.', 1;

IF DB_ID() <> @ExpectedDatabaseId
    THROW 527274, 'ISOLATED_DATABASE_IDENTITY_REJECTED: database_id.', 1;
...
(
    ...
    THROW 527275, 'ISOLATED_DATABASE_IDENTITY_REJECTED: database GUID.', 1;
...
(
    ...
    THROW 527276, 'RT02 isolated database must be ONLINE and READ_WRITE.', 1;
...
(
    ...
    THROW 527277, 'RT02 approved TEST environment marker is missing.', 1;
...
(
    ...
    THROW 527278, 'RT02 owner approval marker is missing.', 1;

IF OBJECT_ID(N'dbo.NguoiLX', N'U') IS NULL
   OR OBJECT_ID(N'dbo.NguoiLX_HoSo', N'U') IS NULL
    THROW 527279, 'RT02 OTO table allowlist preflight failed.', 1;
...
(
    ...
    THROW 527280, 'RT02 OTO allowlisted tables require primary keys.', 1;
GO
...
(
    ...
    THROW 527281, 'RT02 OTO Change Tracking verification failed.', 1;
GO
