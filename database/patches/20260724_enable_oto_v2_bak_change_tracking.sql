/*
    Enable SQL Server Change Tracking for the fixed OTO V2 BAK source database.
    Safety:
...
SET XACT_ABORT ON;
GO
IF DB_NAME() <> N'CSDL_OTO_BAK'
BEGIN
    THROW 527440, 'Wrong database. Expected CSDL_OTO_BAK.', 1;
END;
IF NOT EXISTS
(
    SELECT 1
...
BEGIN
    THROW 527441, 'CSDL_OTO_BAK must be ONLINE and read-write.', 1;
END;
...
BEGIN
    THROW 527442, 'A required CSDL_OTO_BAK Change Tracking table is missing.', 1;
END;
IF EXISTS
(
    SELECT 1
...
BEGIN
    THROW 527443, 'Every required CSDL_OTO_BAK Change Tracking table must have a primary key.', 1;
END;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.DM_DonViGTVT', N'U') AND is_track_columns_updated_on = 0)
    THROW 527444, 'dbo.DM_DonViGTVT is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.KhoaHoc', N'U') AND is_track_columns_updated_on = 0)
    THROW 527446, 'dbo.KhoaHoc is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.BaoCaoI', N'U') AND is_track_columns_updated_on = 0)
    THROW 527448, 'dbo.BaoCaoI is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.NguoiLX', N'U') AND is_track_columns_updated_on = 0)
    THROW 527449, 'dbo.NguoiLX is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.NguoiLX_HoSo', N'U') AND is_track_columns_updated_on = 0)
    THROW 527450, 'dbo.NguoiLX_HoSo is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.NguoiLX_GPLX', N'U') AND is_track_columns_updated_on = 0)
    THROW 527451, 'dbo.NguoiLX_GPLX is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.NguoiLXHS_GiayTo', N'U') AND is_track_columns_updated_on = 0)
    THROW 527452, 'dbo.NguoiLXHS_GiayTo is tracked without TRACK_COLUMNS_UPDATED.', 1;
GO
IF
(
    SELECT COUNT(*)
...
BEGIN
    THROW 527453, 'CSDL_OTO_BAK required Change Tracking verification failed.', 1;
END;
GO