/*
    Enable SQL Server Change Tracking for the fixed MOTO V2 source database.
    Safety:
...
SET XACT_ABORT ON;
GO
IF DB_NAME() <> N'CSDL_MOTO'
BEGIN
    THROW 527420, 'Wrong database. Expected CSDL_MOTO.', 1;
END;
IF NOT EXISTS
(
    SELECT 1
...
BEGIN
    THROW 527421, 'CSDL_MOTO must be ONLINE and read-write.', 1;
END;
...
BEGIN
    THROW 527422, 'A required CSDL_MOTO Change Tracking table is missing.', 1;
END;
IF EXISTS
(
    SELECT 1
...
BEGIN
    THROW 527423, 'Every required CSDL_MOTO Change Tracking table must have a primary key.', 1;
END;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.DM_DonViGTVT', N'U') AND is_track_columns_updated_on = 0)
    THROW 527424, 'dbo.DM_DonViGTVT is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.KhoaHoc', N'U') AND is_track_columns_updated_on = 0)
    THROW 527426, 'dbo.KhoaHoc is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.BaoCaoI', N'U') AND is_track_columns_updated_on = 0)
    THROW 527428, 'dbo.BaoCaoI is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.NguoiLX', N'U') AND is_track_columns_updated_on = 0)
    THROW 527429, 'dbo.NguoiLX is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.NguoiLX_HoSo', N'U') AND is_track_columns_updated_on = 0)
    THROW 527430, 'dbo.NguoiLX_HoSo is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.NguoiLX_GPLX', N'U') AND is_track_columns_updated_on = 0)
    THROW 527431, 'dbo.NguoiLX_GPLX is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.NguoiLXHS_GiayTo', N'U') AND is_track_columns_updated_on = 0)
    THROW 527432, 'dbo.NguoiLXHS_GiayTo is tracked without TRACK_COLUMNS_UPDATED.', 1;
GO
IF
(
    SELECT COUNT(*)
...
BEGIN
    THROW 527433, 'CSDL_MOTO required Change Tracking verification failed.', 1;
END;
GO