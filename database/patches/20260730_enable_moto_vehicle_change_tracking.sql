/*
    ...
    Enables Change Tracking only for the proven CSDL_MOTO.dbo.XeTap source.
...
IF DB_NAME()<>N'CSDL_MOTO' OR DB_ID()<>8 OR NOT EXISTS
(
    SELECT 1
    ...
    THROW 527920, 'VEHICLE_MOTO_SOURCE_IDENTITY_REJECTED', 1;

IF OBJECT_ID(N'dbo.XeTap',N'U') IS NULL
    THROW 527921, 'VEHICLE_MOTO_SOURCE_TABLE_MISSING', 1;

IF (SELECT COUNT(1) FROM sys.columns
    WHERE object_id=OBJECT_ID(N'dbo.XeTap'))<>34
    THROW 527922, 'VEHICLE_MOTO_SOURCE_COLUMN_COUNT_MISMATCH', 1;
...
(
    ...
    THROW 527923, 'VEHICLE_MOTO_SOURCE_PRIMARY_KEY_MISMATCH', 1;
...
(
    ...
    ALTER DATABASE [CSDL_MOTO] SET CHANGE_TRACKING=ON
...
(
    ...
    THROW 527924, 'VEHICLE_MOTO_TRACK_COLUMNS_UPDATED_REQUIRED', 1;
GO
...
IF CHANGE_TRACKING_MIN_VALID_VERSION(OBJECT_ID(N'dbo.XeTap')) IS NULL
    THROW 527925, 'VEHICLE_MOTO_CHANGE_TRACKING_VERIFICATION_FAILED', 1;

SELECT N'VEHICLE_MOTO_CT_READY_CHECKPOINT_NOT_INITIALIZED' AS Result,
