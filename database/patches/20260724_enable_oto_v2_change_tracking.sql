/*
    Enable SQL Server Change Tracking for the fixed OTO V2 source database.
    Safety:
...
SET XACT_ABORT ON;
GO
IF DB_NAME() <> N'CSDL_OTO'
BEGIN
    THROW 527400, 'Wrong database. Expected CSDL_OTO.', 1;
END;
IF NOT EXISTS
(
    SELECT 1
...
BEGIN
    THROW 527401, 'CSDL_OTO must be ONLINE and read-write.', 1;
END;
...
BEGIN
    THROW 527402, 'A required CSDL_OTO Change Tracking table is missing.', 1;
END;
IF EXISTS
(
    SELECT 1
...
BEGIN
    THROW 527403, 'Every required CSDL_OTO Change Tracking table must have a primary key.', 1;
END;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.DM_DonViGTVT', N'U') AND is_track_columns_updated_on = 0)
    THROW 527404, 'dbo.DM_DonViGTVT is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.KhoaHoc', N'U') AND is_track_columns_updated_on = 0)
    THROW 527406, 'dbo.KhoaHoc is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.BaoCaoI', N'U') AND is_track_columns_updated_on = 0)
    THROW 527408, 'dbo.BaoCaoI is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.NguoiLX', N'U') AND is_track_columns_updated_on = 0)
    THROW 527409, 'dbo.NguoiLX is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.NguoiLX_HoSo', N'U') AND is_track_columns_updated_on = 0)
    THROW 527410, 'dbo.NguoiLX_HoSo is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.NguoiLX_GPLX', N'U') AND is_track_columns_updated_on = 0)
    THROW 527411, 'dbo.NguoiLX_GPLX is tracked without TRACK_COLUMNS_UPDATED.', 1;
...
ELSE IF EXISTS (SELECT 1 FROM sys.change_tracking_tables WHERE object_id = OBJECT_ID(N'dbo.NguoiLXHS_GiayTo', N'U') AND is_track_columns_updated_on = 0)
    THROW 527412, 'dbo.NguoiLXHS_GiayTo is tracked without TRACK_COLUMNS_UPDATED.', 1;
GO
IF
(
    SELECT COUNT(*)
...
BEGIN
    THROW 527413, 'CSDL_OTO required Change Tracking verification failed.', 1;
END;
GO