...
/* Exact rollback for 20260727_rt03_enable_ct_snapshot_oto_production.sql. */
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
IF CONVERT(nvarchar(128), SERVERPROPERTY(N'ServerName')) <> N'CSDLTTTC'
   ...
   OR NOT EXISTS
    ...
    THROW 527544, 'RT03_PRODUCTION_IDENTITY_REJECTED: CSDL_OTO rollback.', 1;
...
(
    ...
    THROW 527545, 'RT03_SNAPSHOT_ROLLBACK_BLOCKED: active CSDL_OTO snapshot.', 1;
GO
...
ALTER DATABASE [CSDL_OTO] SET CHANGE_TRACKING = OFF;
GO
ALTER DATABASE [CSDL_OTO] SET ALLOW_SNAPSHOT_ISOLATION OFF;
GO
...
IF EXISTS (SELECT 1 FROM sys.change_tracking_tables)
   ...
   OR (SELECT is_read_committed_snapshot_on FROM sys.databases WHERE database_id = DB_ID()) <> 0
    THROW 527546, 'RT03_CT_ROLLBACK_POSTCONDITION_REJECTED: CSDL_OTO.', 1;

SELECT N'RT03_OTO_CT_ROLLBACK_VERIFIED' AS Evidence;
GO
