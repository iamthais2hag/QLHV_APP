/*
    Lossless V2 -> V1 schema compatibility for the fixed CSDL_MOTO_V1 target.
...
SET XACT_ABORT ON;
GO
IF DB_NAME() <> N'CSDL_MOTO_V1'
    THROW 527520, 'Wrong database. Expected CSDL_MOTO_V1.', 1;
IF NOT EXISTS
(
    SELECT 1
    FROM sys.databases
    ...
    THROW 527521, 'CSDL_MOTO_V1 must be ONLINE and read-write.', 1;
IF OBJECT_ID(N'dbo.DM_DonViGTVT', N'U') IS NULL
   OR OBJECT_ID(N'dbo.NguoiLX_HoSo', N'U') IS NULL
    THROW 527522, 'A required CSDL_MOTO_V1 compatibility table is missing.', 1;
GO
BEGIN TRY
    BEGIN TRANSACTION;
    IF NOT EXISTS
    (
        ...
        THROW 527523, 'dbo.DM_DonViGTVT.TenDV has an incompatible definition.', 1;
    IF EXISTS
    (
        SELECT 1
        FROM sys.columns
    ...
    (
        ...
        THROW 527524, 'dbo.DM_DonViGTVT.CoQuanQL has an incompatible definition.', 1;
    IF EXISTS
    (
        SELECT 1
        FROM sys.columns
    ...
    (
        ...
        THROW 527530, 'dbo.NguoiLX_HoSo.QDThucHanhHinh has an incompatible definition.', 1;
    IF NOT EXISTS
    (
        SELECT 1
        FROM sys.columns columnMetadata
        ...
        THROW 527531, 'dbo.NguoiLX_HoSo.SoGiayCNTN must remain nvarchar(30) NULL.', 1;
    COMMIT TRANSACTION;
END TRY
BEGIN CATCH
    IF @@TRANCOUNT > 0
...
BEGIN TRY
    ...
    BEGIN
        ...
        (
            ...
            THROW 527532, 'A V2-only dbo.GiaoVien compatibility column is incompatible.', 1;
        IF EXISTS
        (
            SELECT 1
            FROM sys.columns
