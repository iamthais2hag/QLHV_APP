...
/*
  READ-ONLY: discover the CSDL_OTO training-teacher model.
  This script does not return names, identity documents, addresses or phone numbers.
*/
...
(
    N'GiaoVien', N'KhoaHoc_GiaoVien', N'KhoaHoc_XeTap', N'KhoaHoc', N'LichHoc',
    N'DM_DonViGTVT', N'DM_DVHC', N'DM_HangGPLX', N'DM_HangDT', N'QTHT_NguoiDung'
)
...
WHERE tableRow.name IN (N'GiaoVien', N'KhoaHoc_GiaoVien', N'KhoaHoc_XeTap')
...
SELECT
    ...
    CASE WHEN moduleRow.definition LIKE N'%GiaoVien%' THEN 1 ELSE 0 END AS ReferencesTeacherName,
    CASE WHEN moduleRow.definition LIKE N'%MaGV%' THEN 1 ELSE 0 END AS ReferencesTeacherCode,
    CASE WHEN moduleRow.definition LIKE N'%SoCMT%' THEN 1 ELSE 0 END AS ReferencesIdentityDocument,
...
WHERE objectRow.name LIKE N'%GiaoVien%'
   OR objectRow.name LIKE N'%LichLV%'
   OR moduleRow.definition LIKE N'%GiaoVien%'
   OR moduleRow.definition LIKE N'%MaGV%'
...
WHERE changeTableRow.object_id = OBJECT_ID(N'dbo.GiaoVien');
