...
function New-SqlConnection {
    ...
    $builder['Application Name'] = 'QLHV RT03 V5 ReadOnly Preflight'
...
$sampleApiFailure = $false
for ($index = 0; $index -lt $SampleCount; $index++) {
    $windowsStart = [DateTimeOffset]::UtcNow
    $statusEvidence = Get-CommandEvidence -Arguments @('/query', '/status', '/verbose')
    $status = $statusEvidence.Output
    $sqlSample = Get-TargetSnapshot
    $sqlRow = $sqlSample.Rows[0]
    $apiUtc = $null
    $apiTimeHealth = $null
    $runtimeCheckedAtUtc = $null
    $apiFailure = $null
    try {
        $apiResponse = Invoke-WebRequest -UseBasicParsing -Uri `
            'http://127.0.0.1:8088/api/system/runtime-status' -TimeoutSec 10
        $api = $apiResponse.Content | ConvertFrom-Json
        $apiUtc = [DateTimeOffset]::Parse(
            [string]$apiResponse.Headers['Date'],
            [Globalization.CultureInfo]::InvariantCulture,
            [Globalization.DateTimeStyles]::AssumeUniversal
        ).ToUniversalTime()
        $apiTimeHealth = [string]$api.Time.TimeHealth
        $runtimeCheckedAtUtc = [string]$api.checkedAtUtc
    }
    catch {
        $apiFailure = $_.Exception.Message
        $sampleApiFailure = $true
    }
    $windowsEnd = [DateTimeOffset]::UtcNow
    $sqlUtc = [DateTimeOffset]::new(
        [datetime]::SpecifyKind(
            [datetime]$sqlRow.SqlUtc,
            [DateTimeKind]::Utc))
    $lastSuccessRaw = Get-LineValue -Text $status `
        -Name 'Last Successful Sync Time'
    $source = Get-LineValue -Text $status -Name 'Source'
    $stratumRaw = Get-LineValue -Text $status -Name 'Stratum'
    $pollRaw = Get-LineValue -Text $status -Name 'Poll Interval'
    $errorRaw = Get-LineValue -Text $status -Name 'Last Sync Error'
    $phase = Get-PhaseMilliseconds -Text $status
    $timeSamples.Add([pscustomobject]@{
        Sample = $index + 1
        WindowsUtcStart = $windowsStart.ToString('o')
        WindowsUtcEnd = $windowsEnd.ToString('o')
        SqlUtc = $sqlUtc.ToString('o')
        ApiUtc = if ($null -eq $apiUtc) { $null } else { $apiUtc.ToString('o') }
        ApiUtcAuthority = 'HTTP_DATE_HEADER_FROM_QLHV_API_SERVER'
        RuntimeCheckedAtUtc = $runtimeCheckedAtUtc
        ApiTimeHealth = $apiTimeHealth
        ApiFailure = $apiFailure
        ApiSqlSkewMilliseconds = if ($null -eq $apiUtc) { $null } else {
            [math]::Round(($apiUtc - $sqlUtc).TotalMilliseconds, 3)
        }
        StatusExitCode = $statusEvidence.ExitCode
        Source = $source
        Stratum = Get-LeadingInteger -Value $stratumRaw
        PollInterval = Get-LeadingInteger -Value $pollRaw
        PhaseOffsetMilliseconds = if ($null -eq $phase) { $null } else {
            [math]::Round($phase, 4)
        }
        LastSuccessfulSyncRaw = $lastSuccessRaw
        LastSuccessfulSyncUtc = $(
            $parsedSync = Convert-LocalSyncTimeToUtc -Value $lastSuccessRaw
            if ($null -eq $parsedSync) { $null } else { $parsedSync.ToString('o') }
        )
        LastSyncErrorCode = Get-LeadingInteger -Value $errorRaw
        LastSyncErrorRaw = $errorRaw
    })
...
$allSourcesExact = @($timeSamples | Where-Object {
    $_.Source -ne $ApprovedNtpPeer
}).Count -eq 0
$validStrata = @($timeSamples | Where-Object {
    $null -eq $_.Stratum -or $_.Stratum -lt 1 -or $_.Stratum -gt 15
}).Count -eq 0
$stablePhase = @($timeSamples | Where-Object {
    $null -eq $_.PhaseOffsetMilliseconds -or
    [math]::Abs([double]$_.PhaseOffsetMilliseconds) -gt $warningSkewMilliseconds
}).Count -eq 0
$stableApiSql = -not $sampleApiFailure -and @($timeSamples | Where-Object {
    $null -eq $_.ApiSqlSkewMilliseconds -or
    [math]::Abs([double]$_.ApiSqlSkewMilliseconds) -gt $warningSkewMilliseconds
}).Count -eq 0
...
$stableTimeEvidence = $allSourcesExact -and $validStrata -and
    $stablePhase -and $stableApiSql -and $durableInPast -and
    $windowsMonotonic -and $timeSamples.Count -ge 3
$freshSuccessAfterTimestampedError =
    $null -ne $lastErrorAtUtc -and
    $null -ne $lastSuccessUtc -and
    $lastSuccessUtc -gt $lastErrorAtUtc
$diagnosticClassification = if ($null -eq $lastErrorCode) {
    'UNCLASSIFIED'
} elseif ($lastErrorCode -eq 0 -and $stableTimeEvidence) {
    'CURRENT_SUCCESS'
} elseif ($lastErrorCode -ne 0 -and
    $freshSuccessAfterTimestampedError -and
    $stableTimeEvidence) {
...
$result = [ordered]@{
    ContractVersion = 'RT03_V5_20260731'
    ...
    TimeAuthority = [ordered]@{
        ...
        LastSyncErrorRaw = $lastSample.LastSyncErrorRaw
    ...
    Gates = [ordered]@{
        ...
        LastSyncErrorSafelyClassified =
            $diagnosticClassification -ne 'CURRENT_FAILURE' -and
            $diagnosticClassification -ne 'UNCLASSIFIED'
